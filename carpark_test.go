...
func values() variables {
	defaultValues := variables{
		car0:       &Car{registration: "KA-01-HH-2701", driver_age : 1},
		car1:       &Car{slot: 1, registration: "KA-01-HH-1234", driver_age : 2},
		car2:       &Car{slot: 2, registration: "KA-01-HH-7777", driver_age : 3},
...
func TestCarpark_removeCar(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _,err := tt.carpark.removeCar(tt.args.slotNo); (err != nil) != tt.wantErr {
...
func TestCarpark_getCarsWithAge(t *testing.T) {
	type args struct {
		age int
	}
	tests := []struct {
		...
		{name: "Carpark with car of requested driver age",
			carpark: &Carpark{Map: values().map1, emptySlot: values().emptySlot0, highestSlot: 1, maxSlot: 10},
			args:    args{age: 1},
			want:    []int{1},
		...
		{name: "Carpark without car of requested driver age",
			carpark: &Carpark{Map: values().map2, emptySlot: values().emptySlot1, highestSlot: 2, maxSlot: 10},
			args:    args{age: 2},
			want:    nil,
			want1:   nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1, err := tt.carpark.getCarsWithAge(tt.args.age)
			if (err != nil) != tt.wantErr {
				t.Errorf("Carpark.getCarsWithAge() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("Carpark.getCarsWithAge() got = %v, want %v", got, tt.want)
			}
			if !reflect.DeepEqual(got1, tt.want1) {
				t.Errorf("Carpark.getCarsWithAge() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
...
func TestCarpark_getCarWithRegistrationNo(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Carpark with car of requested regis No.",
			carpark: &Carpark{Map: values().map1, emptySlot: values().emptySlot0, highestSlot: 1, maxSlot: 10},
			args:    args{registration: "KA-01-HH-1234"},
			want:    1,
			wantErr: false,
		},
		{name: "Carpark without car of requested regis No.",
			carpark: &Carpark{Map: values().map2, emptySlot: values().emptySlot1, highestSlot: 2, maxSlot: 10},
			args:    args{registration: "KA-01-HH-1234"},
			want:    0,
			wantErr: true,
